# Photo credits

The original stock photographs below are from Pexels. Generated replacements are listed separately; original files are retained for reference.

## Generated replacements — September 18, 2026

Created with the built-in OpenAI image generation tool. These are illustrative scenes, not documentation of Nextech personnel, premises, inventory or projects. The Abu Dhabi banner is an architectural illustration, not a location photograph. Prompts are recorded in `design/generated-image-prompts.json`.

- `services/instrumentation-control-generated.webp` — instrument calibration scene.
- `divisions/chemicals-safety-equipment-generated.webp` — industrial PPE and chemical-storage setting.
- `divisions/mechanical-flow-control-generated.webp` — industrial valve and process pipework.
- `inspection-generated.webp` — valve inspection before dispatch.
- `supply-partnership-generated.webp` — industrial equipment sourcing collaboration.
- `sourcing-story-generated.webp` — technical sourcing desk and equipment samples.
- `abu-dhabi-illustration-generated.webp` — Abu Dhabi-inspired waterfront illustration.

## Original assets

- `hero-offshore.webp` — https://www.pexels.com/photo/16830015/
- `engineers-site.webp` — https://www.pexels.com/photo/3158651/
- `divisions/mechanical-flow-control.webp` — https://www.pexels.com/photo/37589838/
- `divisions/electrical.webp` — https://www.pexels.com/photo/34526423/
- `divisions/instrumentation.webp` — https://www.pexels.com/photo/13401910/
- `divisions/heavy-process-industrial-equipment.webp` — https://www.pexels.com/photo/15970032/
- `divisions/chemicals-safety-equipment.webp` — https://www.pexels.com/photo/29083058/
- `sustainability.webp` — https://www.pexels.com/photo/18829033/
- `page-hero.webp` — https://www.pexels.com/photo/15970027/
- `industries/oil-and-gas.webp` — https://www.pexels.com/photo/37703372/
- `industries/water.webp` — https://www.pexels.com/photo/27490881/
- `industries/petrochemical.webp` — https://www.pexels.com/photo/6537735/
- `industries/utility.webp` — https://www.pexels.com/photo/16705829/
- `industries/marine.webp` — https://www.pexels.com/photo/21234960/
- `industries/energy.webp` — https://www.pexels.com/photo/8853509/
- `services/civil-works-mechanical-services.webp` — https://www.pexels.com/photo/12314551/
- `services/electrical-power-systems.webp` — https://www.pexels.com/photo/33694016/
- `services/instrumentation-control.webp` — https://www.pexels.com/photo/4889280/
- `services/engineering-services.webp` — https://www.pexels.com/photo/3862377/
- `services/consultancy-advisory.webp` — https://www.pexels.com/photo/5816300/
- `services/project-management.webp` — https://www.pexels.com/photo/8961073/
- `who-we-are-story.webp` — previous generated meeting image; retained but replaced on the site by the technical sourcing scene.
- `inspection.webp` — https://www.pexels.com/photo/37466061/
- `abu-dhabi.webp` — https://www.pexels.com/photo/19612315/
- `earth-night.webp` — https://www.pexels.com/photo/30596225/
